...
topk_metrics = {metric.lower(): metric for metric in ['Recall', 'Recall2', 'Precision', 'NDCG', 'MAP', 'Gini']}
...
class TopKEvaluator(object):
    ...
    def fine_grained_evaluate(self, batch_matrix_list, eval_data, user_group_dict, item_group_dict):
        pos_items = eval_data.get_eval_items()
        pos_len_list = eval_data.get_eval_len_list()
        topk_index = torch.cat(batch_matrix_list, dim=0).cpu().numpy()
        bool_rec_matrix = []
        for m, n in zip(pos_items, topk_index):
            bool_rec_matrix.append([True if i in m else False for i in n])
        bool_rec_matrix = np.asarray(bool_rec_matrix)  # （n_user， k)

        if user_group_dict is None:
            user_result_dict = {}
        else:
            user_result_dict = {}
            # 按照user_group_dict进行分组计算metrics
            for k, v in user_group_dict.items():
                metric_dict = {}
                result_list = self._calculate_metrics(pos_len_list[v], bool_rec_matrix[v])
                for metric, value in zip(self.metrics, result_list):
                    for i in self.topk:
                        key = '{}@{}'.format(metric, i)
                        metric_dict[key] = round(value[i - 1], 4)
                user_result_dict[k] = metric_dict

        if item_group_dict is None:
            item_result_dict = {}
        else:
            topk = 20
            # Step 1: 建立 item -> group 映射
            item2group = {}
            for g, items in item_group_dict.items():
                for it in items:
                    item2group[it] = g

            # Step 2: 截取前 topk 推荐并展平
            items = topk_index[:, :topk].ravel()

            # Step 3: 映射成 group，并统计频次
            groups = [item2group[it] for it in items if it in item2group]
            counter = Counter(groups)

            # Step 4: 转换为比例
            total = sum(counter.values())
            item_result_dict = {g: round(counter.get(g, 0) / total, 4) for g in item_group_dict.keys()}

        return user_result_dict, item_result_dict
