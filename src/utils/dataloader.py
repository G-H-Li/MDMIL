...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _get_neg_sample(self):
        ...
        self.pr += self.step
        # to tensor
        user_tensor = torch.tensor(cur_data[self.config['USER_ID_FIELD']].values).type(torch.int64).to(self.device)
        item_tensor = torch.tensor(cur_data[self.config['ITEM_ID_FIELD']].values).type(torch.int64).to(self.device)
    ...
    def _get_full_uids_sample(self):
        user_tensor = torch.tensor(self.all_uids[self.pr: self.pr + self.step]).type(torch.int64).to(self.device)
        self.pr += self.step
        return user_tensor
    ...
    def _sample_neg_ids(self, u_ids):
        ...
        for u in u_ids:
            ...
            while iid in self.history_items_per_u[u]:
                iid = self._random()
            neg_ids.append(iid)
        return torch.tensor(neg_ids).type(torch.int64)
    ...
    def _get_neighborhood_samples(self, ids, id_str):
        ...
        return torch.tensor(pos_ids).type(torch.int64), torch.tensor(neg_ids).type(torch.int64)
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        # to device
        self.eval_u = torch.tensor(self.eval_u).type(torch.int64).to(self.device)
    ...
    def _get_pos_items_per_u(self, eval_users):
        ...
        if self.config['cold_start_type'] != 'none':
            return torch.zeros((2, 0), dtype=torch.int64).to(self.device)
        ...
        for i, u in enumerate(eval_users):
            ...
            i_ids.extend(u_ls)
        return torch.tensor([u_ids, i_ids]).type(torch.int64)
